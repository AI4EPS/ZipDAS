...
def benchmarking(data_type, method, keep_ratio, quality):
    ...
    if data_type == "noise":
        ...
        # cc_cmd = f"python CCTorch/run.py --data_list1=noise_data.txt --data_path={config.decomp_path}_{config.keep_ratio:.2f}_{config.quality:.0f}/{config.method}  --mode=AN  --block_size1 1 --block_size2 100 --fixed_channels 100 300 500 700 900  --dt=0.02 --maxlag=15 --result_path {config.decomp_path}_{config.keep_ratio:.2f}_{config.quality:.0f}/{config.method}/cctorch"
        # print(cc_cmd)
        # os.system(cc_cmd)

        plot_cmd = f"python CCTorch/tests/test_ambient_noise.py --result_path {config.decomp_path}_{config.keep_ratio:.2f}_{config.quality:.0f}/{config.method}/cctorch --figure_path {config.decomp_path}_{config.keep_ratio:.2f}_{config.quality:.0f}/{config.method}/cctorch  --fixed_channels 100 300 500 700 900"
        print(plot_cmd)
        os.system(plot_cmd)
...
if __name__ == "__main__":
    ...
    # os.system("python run.py --mode compress --data_path noise_data --data_format h5 --result_path results/compressed_noise --method=jpeg --save_preprocess --quality=1.0")
